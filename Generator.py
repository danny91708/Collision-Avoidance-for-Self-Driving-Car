...
class DriveDataGenerator(image.ImageDataGenerator):
    ...
    def random_transform_with_states(self, x, seed=None):
        ...
        if self.rotation_range:
            theta = np.pi / 180 * np.random.uniform(-1*self.rotation_range, self.rotation_range)
        else:
            theta = 0

        if self.height_shift_range:
            tx = np.random.uniform(-1*self.height_shift_range, self.height_shift_range) * x.shape[img_row_axis]
        else:
            tx = 0

        if self.width_shift_range:
            ty = np.random.uniform(-1*self.width_shift_range, self.width_shift_range) * x.shape[img_col_axis]
        else:
            ty = 0

        if self.shear_range:
            shear = np.random.uniform(-1*self.shear_range, self.shear_range)
        else:
            shear = 0
        ...
        if transform_matrix is not None:
            ...
            x = image.apply_transform(x, transform_matrix, img_channel_axis,  fill_mode=self.fill_mode, cval=self.cval)
